...
// atomicWriteJSON writes the fragment atomically (temp file + rename so a
// SIGKILL between write and rename can't leak a partial file under a
// deterministic name). Unlike codex/copilot, cursor uses 0o755 conversation
// dirs (its layout is conversation-scoped, not a flat private state dir); the
// file itself stays 0o600.
func atomicWriteJSON(target string, v any) error {
	return fragmentstore.WriteJSON(target, v, fragmentstore.WriteOptions{DirMode: 0o755, FileMode: 0o600})
}
