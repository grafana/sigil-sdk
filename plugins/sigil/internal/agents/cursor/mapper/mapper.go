...
func buildMessages(frag *fragment.Fragment, mode sigil.ContentCaptureMode) (input, output []sigil.Message) {
	...
	// that build Inputs directly. NormalizeContentMode also collapses
	// FullWithMetadataSpans → Full because the two modes differ only in OTel
	// span exposure; the gRPC payload buildMessages produces is identical.
	mode = mapperutil.NormalizeContentMode(mode)
