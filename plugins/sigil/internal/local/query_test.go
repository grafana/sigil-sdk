...
func TestDisjointTokenUsage(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			fresh, cr, cw, out, reas := disjointTokenUsage(tc.usage, tc.provider)
			assert.Equal(t, tc.freshInput, fresh, "fresh_input")
			assert.Equal(t, tc.cacheRead, cr, "cache_read")
			assert.Equal(t, tc.cacheWrite, cw, "cache_write")
			assert.Equal(t, tc.output, out, "output")
			assert.Equal(t, tc.reasoning, reas, "reasoning")
		})
	}
}
