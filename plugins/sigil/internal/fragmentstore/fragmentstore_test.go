...
func TestWriteJSONRoundTripsAndCleansTempFiles(t *testing.T) {
	...
	if err := WriteJSON(target, rec{Name: "a", Count: 1}, WriteOptions{DirMode: 0o700, FileMode: 0o600}); err != nil {
...
func TestWriteJSONPermissions(t *testing.T) {
	...
	cases := []struct {
		...
		opts     WriteOptions
		wantDir  os.FileMode
		wantFile os.FileMode // 0 = don't check
	}{
		{
			name:     "tight perms (codex/copilot policy)",
			opts:     WriteOptions{DirMode: 0o700, FileMode: 0o600},
			wantDir:  0o700,
			wantFile: 0o600,
		},
		{
			name:    "loose dir (cursor policy)",
			opts:    WriteOptions{DirMode: 0o755},
			wantDir: 0o755,
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			dir := filepath.Join(t.TempDir(), "nested", "dir")
			target := filepath.Join(dir, "frag.json")
			if err := WriteJSON(target, rec{Name: "a", Count: 1}, tc.opts); err != nil {
				t.Fatalf("WriteJSON: %v", err)
			}
			di, err := os.Stat(dir)
			if err != nil {
			...
			if perm := di.Mode().Perm(); perm != tc.wantDir {
				t.Errorf("dir mode = %o; want %o", perm, tc.wantDir)
			}
			if tc.wantFile != 0 {
				...
				if perm := fi.Mode().Perm(); perm != tc.wantFile {
					t.Errorf("file mode = %o; want %o", perm, tc.wantFile)
				}
			}
		})
	}
