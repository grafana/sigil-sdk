...
test('context propagation: generation callback sets mode for child tool executions', async () => {
  const harness = newHarness({ contentCapture: 'metadata_only' });
  try {
    await harness.client.startGeneration(
      { model: { provider: 'test', name: 'test-model' } },
      async (recorder) => {
...
const generationModeTests = [
  { name: 'client default, gen default → no_tool_content', clientMode: undefined, genMode: undefined, wantStripped: false, wantMarker: 'no_tool_content' },
  { name: 'client metadata_only, gen default → stripped', clientMode: 'metadata_only', genMode: undefined, wantStripped: true, wantMarker: 'metadata_only' },
  { name: 'client full, gen metadata_only → stripped', clientMode: 'full', genMode: 'metadata_only', wantStripped: true, wantMarker: 'metadata_only' },
  { name: 'client metadata_only, gen full → full', clientMode: 'metadata_only', genMode: 'full', wantStripped: false, wantMarker: 'full' },
  { name: 'client full, gen default → full', clientMode: 'full', genMode: undefined, wantStripped: false, wantMarker: 'full' },
];
...
test('metadata_only strips message.content shorthand field', async () => {
  ...
  try {
    ...
    recorder.setResult({
      input: [
        { role: 'user', content: 'Hello secret data', parts: [{ type: 'text', text: 'Hello text' }] },
      ],
      output: [
        { role: 'assistant', content: 'Secret response', parts: [{ type: 'text', text: 'Response text' }] },
      ],
      usage: { inputTokens: 10, outputTokens: 5 },
    });
    recorder.end();
...
test('metadata_only validates content-only messages (no parts) after stripping', async () => {
  ...
  try {
    ...
    recorder.setResult({
      input: [
        { role: 'user', content: 'Hello with no parts' },
      ],
      output: [
...
test('resolver returning invalid mode defaults to metadata_only', async () => {
  ...
  const harness = newHarness({
    ...
    logger: { warn: (msg) => { warnMessage = msg; } },
  });
  try {
    const recorder = harness.client.startGeneration({
      model: { provider: 'test', name: 'test-model' },
...
test('resolver throwing defaults to metadata_only with warning', async () => {
  let warnMessage = '';
  const harness = newHarness({
    contentCaptureResolver: () => { throw new Error('resolver broken'); },
    logger: { warn: (msg) => { warnMessage = msg; } },
  });
  try {
    const recorder = harness.client.startGeneration({
