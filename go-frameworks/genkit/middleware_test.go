...
func TestCallError(t *testing.T) {
	...
	input := arrayValue(t, gen, "input")
	if len(input) != 1 {
		t.Fatalf("expected 1 input message on error path, got %d", len(input))
	}
	inputMsg := asObject(t, input[0], "input[0]")
	requireStringField(t, inputMsg, "role", "MESSAGE_ROLE_USER")
}
...
func (c *generationCaptureServer) SingleGeneration(t *testing.T) map[string]any {
	...
	gen := asObject(t, generations[0], "generations[0]")
	requireBaseFields(t, gen)
	return gen
}

// requireBaseFields asserts fields that every exported generation must have,
// regardless of whether the model call succeeded or failed.
func requireBaseFields(t *testing.T, gen map[string]any) {
	t.Helper()

	op := stringValue(t, gen, "operation_name")
	if op != "generateText" && op != "streamText" {
		t.Fatalf("operation_name: got %q, want generateText or streamText", op)
	}

	model := objectValue(t, gen, "model")
	if v := stringValue(t, model, "provider"); v == "" {
		t.Fatal("model.provider is empty")
	}
	if v := stringValue(t, model, "name"); v == "" {
